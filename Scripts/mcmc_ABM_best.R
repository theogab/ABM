...
# pars : vector with /[1] sig2 /[2] rho /[3] delta
lik_ABM <- function(tab, x, pars){
  ...
  rg1 <- dnorm(x[tab[,2],2], rnge(omega, nu, x[tab[,1],2],F), sqrt(sig2[2] * tab[,4])) * dnorm(x[tab[,3],2], rnge(omega, nu, x[tab[,1],2],T), sqrt(sig2[2] * tab[,5]))
  mp1 <- dnorm(x[tab[,2],1], midpt(omega, nu, x[tab[,1],],F,F), sqrt(sig2[1] * tab[,4])) * dnorm(x[tab[,3],1], midpt(omega, nu, x[tab[,1],],T,T), sqrt(sig2[1] * tab[,5]))
  # scenario 2 -> sp1 inherits high midpoint and wide range
  rg2 <- dnorm(x[tab[,2],2], rnge(omega, nu, x[tab[,1],2],T), sqrt(sig2[2] * tab[,4])) * dnorm(x[tab[,3],2], rnge(omega, nu, x[tab[,1],2],F), sqrt(sig2[2] * tab[,5]))
  mp2 <- dnorm(x[tab[,2],1], midpt(omega, nu, x[tab[,1],],T,F), sqrt(sig2[1] * tab[,4])) * dnorm(x[tab[,3],1], midpt(omega, nu, x[tab[,1],],F,T), sqrt(sig2[1] * tab[,5]))
  # scenario 3 -> sp1 inherits low midpoint and narrow range
  mp3 <- dnorm(x[tab[,2],1], midpt(omega, nu, x[tab[,1],],F,T), sqrt(sig2[1] * tab[,4])) * dnorm(x[tab[,3],1], midpt(omega, nu, x[tab[,1],],T,F), sqrt(sig2[1] * tab[,5]))
  # scenario 4 -> sp1 inherits low midpoint and wide range
  mp4 <- dnorm(x[tab[,2],1], midpt(omega, nu, x[tab[,1],],T,T), sqrt(sig2[1] * tab[,4])) * dnorm(x[tab[,3],1], midpt(omega, nu, x[tab[,1],],F,F), sqrt(sig2[1] * tab[,5]))
  
  lik <- sum(log(max(mp1*rg1,mp2*rg2,mp3*rg1,mp4*rg2)))
...
rnge <- function(omega, nu, X0, wide = F){
  if(wide) return(X0/2*(omega*(1-nu^2)/(1+nu)+1+nu))
  else return(X0/2*(omega*(1-nu^2)/(1+nu)+1-nu))
}
...
make_ABM <- function(phy, traits, scale = F){
  ...
  ### Global variables ###
  ABM$data$traits <- traits[phy$tip.label,]
  ...
  ABM$init <- c(runif(2, 0.1, 3), 0.5, 0.5)
  ABM$proposals <- list()
  ABM$priors <- list()
  for (i in 1:ABM$data$tree$Nnode) {
    ABM$priors[[i]] <- jive:::hpfun("Normal", c(ABM$nodes[i,1],.5))
    ABM$priors[[i+ABM$data$tree$Nnode]] <- jive:::hpfun("Lognormal", c(log(ABM$nodes[i,2]),.5))
  }
  ABM$proposals[[1]] <- jive:::proposal("slidingWinAbs")
  ABM$proposals[[2]] <- jive:::proposal("multiplierProposal")
