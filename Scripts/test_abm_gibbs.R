...
ABM.obj$update.freq <- c(0.2, 0, 0, 0.1, 0.1, 0.1, 0.1, 0.2, 0.2)
lik_ABM(tree = ABM.obj$data$tree, logX = sim$logX, m = sim$m, pars = c(sm, sx, nu, om), Sx = ABM.obj$init$Sx, Sm = ABM.obj$init$Sm)

mcmc_ABM(ABM.obj, log.file = sprintf("Results/abm_gibbs_n-%s_sm-%s_sx-%s_nu-%s_om-%s_sim-%s.log", n, sm, sx, nu, om, sim_nb), sampling.freq = 1, print.freq = 1, ngen = 500)
